...
If this becomes continuous ("thrashing"), `dpowcoind` can slow to a crawl, especially during initial sync or reindex.

If you see sustained swap I/O while `dpowcoind` runs, restart with a lower `-dbcache`.
If needed, also reduce `-maxmempool`, `-maxconnections`, or use `-blocksonly`.
Dpowcoin Core may warn at startup when `-dbcache` looks too large for the detected system memory.
