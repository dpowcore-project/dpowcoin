...
Dpowcoin Core version 26.0 is now available from:

  <https://bitcoincore.org/bin/dpowcoin-core-26.0/>
...
- Experimental support for the v2 transport protocol defined in
  [BIP324](https://github.com/dpowcoin/bips/blob/master/bip-0324.mediawiki) was added.
...
- [Miniscript](https://dpowcoin.sipa.be/miniscript/) expressions can now be used in Taproot descriptors for all RPCs working with descriptors. (#27255)

- `finalizepsbt` is now able to finalize a PSBT with inputs spending [Miniscript](https://dpowcoin.sipa.be/miniscript/)-compatible Taproot leaves. (#27255)
...
- `loadtxoutset` has been added, which allows loading a UTXO snapshot of the format
  ...
  document (<https://github.com/dpowcoin/dpowcoin/blob/master/doc/design/assumeutxo.md>).
...
- `dpowcoind` and `dpowcoin-qt` will now raise an error on startup
 if a datadir that is being used contains a dpowcoin.conf file that
 will be ignored, which can happen when a datadir= line is used in
 a dpowcoin.conf file. The error message is just a diagnostic intended
 ...
 the dpowcoin.conf contained in it. (#27302)
...
- Wallet loading has changed in this release. Wallets with some corrupted records that could be
  ...
  load the wallet in a previous version of Dpowcoin Core and import the data into a new wallet.
...
```sh
src/dpowcoin-cli -named bumpfee txid fee_rate=100
```

instead of

```sh
src/dpowcoin-cli -named bumpfee txid options='{"fee_rate": 100}'
```
...
- It's now possible to use [Miniscript](https://dpowcoin.sipa.be/miniscript/) inside Taproot leaves for descriptor wallets. (#27255)
...
- The usage of hybrid public keys in output descriptors has been removed. Hybrid
  ...
  (as specified in BIP380 and documented in doc/descriptors.md). Dpowcoin Core would
...
- Bash completion files have been renamed from `dpowcoin*.bash-completion` to
  `dpowcoin*.bash`. This means completions can be automatically loaded on demand
