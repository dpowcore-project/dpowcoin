...
Dpowcoin Core version 0.20.0 is now available from:

  <https://bitcoincore.org/bin/dpowcoin-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Dpowcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Dpowcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Dpowcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Dpowcoin Core. Dpowcoin Core logs debug messages
...
- OpenSSL is no longer used by Dpowcoin Core.  (#17265)

- BIP70 support has been fully removed from Dpowcoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/dpowcoin/dpowcoin/issues/16599),
  [PR #16702](https://github.com/dpowcoin/dpowcoin/pull/16702), and the
  `dpowcoind help` for more information.  This option is experimental and
...
- All custom settings configured when Dpowcoin Core starts are now
...
- The "Start Dpowcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple Dpowcoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Dpowcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/dpowcoin-core/HWI) for
  signing.  Future versions of Dpowcoin Core should support a GUI option
...
- Dpowcoin Core's automatically-generated source code documentation is
...
- The `dpowcoin-cli` utility used with the `-getinfo` parameter now
...
- The `dpowcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17741 Included `test_dpowcoin-qt` in msvc build (sipsorcery)
...
- #18702 Fix ASLR for dpowcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate dpowcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest dpowcoin.conf (sdaftuar)
...
- #18166 ci: Run fuzz testing test cases (dpowcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone dpowcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Dpowcoin Core files (hebasto)
...
- #17119 Fix broken dpowcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from dpowcoin-qt help message and manpage (laanwj)
...
- #17648 Rename wallet-tool references to dpowcoin-wallet (hel-o)
...
- #18341 Replace remaining literal DPC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On dpowcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from dpowcoin-cli -getinfo (malevolent)
