Dpowcoin Core version 28.0 is now available from:

  <https://bitcoincore.org/bin/dpowcoin-core-28.0/>
...
Support for Testnet4 as specified in [BIP94](https://github.com/dpowcoin/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\Dpowcoin`
to `C:\Users\Username\AppData\Local\Dpowcoin`. Dpowcoin Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/dpowcoin/dpowcoin/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
- Previously if Dpowcoin Core was listening for P2P connections, either using
...
- Dpowcoin Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/dpowcoin/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Dpowcoin Core with the configuration
...
are now required to compile Dpowcoin Core. (#29091, #30263)

- The minimum required glibc to run Dpowcoin Core is now
...
Previous releases of Dpowcoin Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
