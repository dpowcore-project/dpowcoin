Dpowcoin Core version 0.11.0 is now available from:

  <https://dpowcoin.org/bin/dpowcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer DPC/kB fee to be
...
For example, add the following to `dpowcoin.conf`:
...
related to the blockchain in the dpowcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Dpowcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Dpowcoin Core will 
...
For an example script see [dpowcoin-submittx](https://github.com/laanwj/dpowcoin-submittx).
...
- #6269 `95aca44` gitian: Use the new dpowcoin-detached-sigs git repo for OSX signatures
...
- #5753 `0f44672` Add dpowcoin logo to about screen
...
- #5809 `46bfbe7` Add dpowcoin-cli man page
...
- #5849 `41259ca` contrib/init/dpowcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Dpowcoin-Qt.app to Dpowcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for dpowcoin-tx)
