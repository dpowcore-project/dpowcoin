...
Dpowcoin Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Dpowcoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/dpowcoin/dpowcoin/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full build/bin/dpowcoind -printtoconsole
...
llvm-cov show \
    ...
    --project-title="Dpowcoin Core Coverage Report"
```
...
llvm-cov show \
    ...
    --show-expansions \
    --output-dir=build/coverage_report \
    --project-title="Dpowcoin Core Fuzz Coverage Report"
```
...
Dpowcoin Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`dpowcoind`)](https://doxygen.bitcoincore.org/dpowcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `dpowcoind.cpp`. Responsible for starting up and
...
pay attention to for reviewers of Dpowcoin Core code.

## General Dpowcoin Core
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers (hh, h, l, ll, j, z, t, L) for arithmetic types.
  ...
  - *Rationale*: Dpowcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Normally, these are maintained by active developers of Dpowcoin Core, in which case
...
a Dpowcoin Core subtree, so that they can be integrated quickly. Cosmetic changes
...
The tool instructions also include a list of the subtrees managed by Dpowcoin Core.
...
- src/leveldb
  - Upstream at https://github.com/google/leveldb ; maintained by Google. Open
    important PRs to the Dpowcoin Core subtree to avoid delay.
...
file descriptors, it will cause problems with Dpowcoin's `select()` loop, because
...
compatibility between nodes. This happened in Dpowcoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Dpowcoin's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/dpowcoin/dpowcoin/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/dpowcoin/dpowcoin/commit/8922d7f6b751a3e6b3b9f6fb7961c442877fb65a)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/dpowcoin/dpowcoin/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Dpowcoin Core, the wallet was always
...
- Interface method names should be `lowerCamelCase` and standalone function names should be
  ...
  *Rationale*: consistency and friendliness to code generation tools.
  ...
  before in [#14635](https://github.com/dpowcoin/dpowcoin/pull/14635).
