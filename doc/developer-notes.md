...
```shell
$ valgrind --suppressions=test/sanitizer_suppressions/valgrind.supp build/bin/test_dpowcoin
$ valgrind --suppressions=test/sanitizer_suppressions/valgrind.supp --leak-check=full \
      --show-leak-kinds=all build/bin/test_dpowcoin --log_level=test_suite
...
# A coverage report will now be accessible at `./build/test_dpowcoin.coverage/index.html`,
...
```shell
llvm-cov show \
    --object=build/bin/test_dpowcoin \
