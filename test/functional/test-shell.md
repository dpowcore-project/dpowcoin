...
We can import the `TestShell` by adding the path of the configured Dpowcoin Core
...
```
>>> import sys
>>> sys.path.insert(0, "/path/to/dpowcoin/build/test/functional")
...
* `/path/to/dpowcoin/build/test/functional/combine_logs.py
...
|---|---|---|
| `bind_to_localhost_only` | `True` | Binds dpowcoind P2P services to `127.0.0.1` if set to `True`.|
| `cachedir` | `"/path/to/dpowcoin/build/test/cache"` | Sets the dpowcoind datadir directory. |
| `chain`  | `"regtest"` | Sets the chain-type for the underlying test dpowcoind processes. |
| `configfile` | `"/path/to/dpowcoin/build/test/config.ini"` | Sets the location of the test framework config file. |
