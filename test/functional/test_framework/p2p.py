...
MAGIC_BYTES = {
    ...
    "signet": b"\xea\x64\xee\x7f",    # signet
}
...
class P2PInterface(P2PConnection):
    ...
    def wait_until(self, test_function_in, *, timeout=300, check_connected=True):
        def test_function():
            if check_connected:
    ...
    def wait_for_connect(self, timeout=300):
        test_function = lambda: self.is_connected
        self.wait_until(test_function, timeout=timeout, check_connected=False)

    def wait_for_disconnect(self, timeout=300):
        test_function = lambda: not self.is_connected
        self.wait_until(test_function, timeout=timeout, check_connected=False)

    # Message receiving helper methods

    def wait_for_tx(self, txid, timeout=300):
        def test_function():
    ...
    def wait_for_block(self, blockhash, timeout=300):
        def test_function():
            return self.last_message.get("block") and self.last_message["block"].block.rehash() == blockhash

        self.wait_until(test_function, timeout=timeout)

    def wait_for_header(self, blockhash, timeout=300):
        def test_function():
    ...
    def wait_for_merkleblock(self, blockhash, timeout=300):
        def test_function():
    ...
    def wait_for_getdata(self, hash_list, timeout=300):
    ...
    def wait_for_getheaders(self, timeout=300):
    ...
    def wait_for_inv(self, expected_inv, timeout=300):
    ...
    def wait_for_verack(self, timeout=300):
        def test_function():
    ...
    def send_and_ping(self, message, timeout=300):
        self.send_message(message)
        self.sync_with_ping(timeout=timeout)

    def sync_with_ping(self, timeout=300):
