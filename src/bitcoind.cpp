...
static bool ParseArgs(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/dpowcoin.conf are parsed in qt/bitcoin.cpp's main()
...
static bool ProcessInitCommands(interfaces::Init& init, ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        if (args.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The " CLIENT_NAME " daemon (dpowcoind) is a headless program that connects to the Dpowcoin network to validate and relay transactions and blocks, as well as relaying addresses.\n\n"
                "It provides the backbone of the Dpowcoin network and its RPC, REST and ZMQ services can provide various transaction, block and address-related services.\n\n"
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for dpowcoind but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect dpowcoind signal handlers
    noui_connect();
