...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " dpowcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The dpowcoin-tx tool is used for creating and modifying dpowcoin transactions.\n\n"
                "dpowcoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded dpowcoin transaction, or with \"-create [commands]\" to create a hex-encoded dpowcoin transaction.\n"
                "\n"
                "Usage: dpowcoin-tx [options] <hex-tx> [commands]\n"
                "or:    dpowcoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded dpowcoin transaction
