...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader * /*pblock*/, const Consensus::Params& params)
{
    ...
    // Dpowcoin uses LWMA-1 (retargets every block), replacing Bitcoin's
    // 2016-block cadence entirely. See Lwma3CalculateNextWorkRequired below.
...
//
// SECURITY-FIXES.md C4 — LWMA-aware per-block clamp.
//
// Bitcoin's original check is tied to the 2016-block retarget cadence and is
// meaningless for LWMA-1, which retargets every block. Without any clamp a
// peer can spam headers-sync with a fake chain of arbitrary low-work targets
// (anything ≤ powLimit) and force us to build a huge in-memory block index
// before the first real PoW check. The clamp here caps the per-block target
// change at a factor of K = 4 in either direction, which is ~10x beyond what
// LWMA-1 can produce on an honest chain (≈30% per block) yet forces any fake
// low-work chain to grow at most K^N slower in cumulative work than claimed.
bool PermittedDifficultyTransition(const Consensus::Params& params, int64_t height, uint32_t old_nbits, uint32_t new_nbits)
{
    if (params.fPowAllowMinDifficultyBlocks) return true;

    // Bootstrap window — the LWMA algorithm itself returns powLimit until the
    // L=577 warm-up completes (see Lwma3CalculateNextWorkRequired), so we must
    // not reject the legitimate flat section here.
    if (height <= params.lwmaAveragingWindow) return true;

    bool neg, over;
    arith_uint256 old_target, new_target;
    old_target.SetCompact(old_nbits, &neg, &over);
    if (neg || over || old_target == 0) return false;
    new_target.SetCompact(new_nbits, &neg, &over);
    if (neg || over || new_target == 0) return false;

    const arith_uint256 pow_limit = UintToArith256(params.powLimit);
    if (new_target > pow_limit) return false;

    // Per-block factor-K clamp. K=4 is deliberately loose: honest LWMA output
