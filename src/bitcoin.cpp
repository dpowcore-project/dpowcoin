...
Options:
  -m, --multiprocess     Run multiprocess binaries dpowcoin-node, dpowcoin-gui.
  -M, --monolithic       Run monolithic binaries dpowcoind, dpowcoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'dpowcoin-qt [ARGS]' or 'dpowcoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'dpowcoind [ARGS]' or 'dpowcoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'dpowcoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'dpowcoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'dpowcoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_dpowcoin [ARGS]'.
  chainstate [ARGS] Run dpowcoin kernel chainstate util, equivalent to running 'dpowcoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_dpowcoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_dpowcoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "dpowcoin-gui" : "dpowcoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "dpowcoin-node" : "dpowcoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("dpowcoin-cli");
            // Since "dpowcoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("dpowcoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("dpowcoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_dpowcoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("dpowcoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_dpowcoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_dpowcoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("dpowcoin-util");
        } else {
...
//! Execute the specified dpowcoind, dpowcoin-qt or other command line in `args`
...
//!             be a relative path to a dpowcoind, dpowcoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/dpowcoin ...
