...
static void mineBlock(const node::NodeContext& node, std::chrono::seconds block_time)
{
    ...
    // Dpowcoin Dual PoW: block must satisfy BOTH yespower AND argon2id under nBits.
    while (!CheckProofOfWork(block.GetYespowerPoWHash(), block.nBits, node.chainman->GetConsensus()) ||
           !CheckProofOfWork(block.GetArgon2idPoWHash(), block.nBits, node.chainman->GetConsensus())) {
        ++block.nNonce;
    }
