...
std::vector<std::shared_ptr<CBlock>> CreateBlockChain(size_t total_height, const CChainParams& params)
{
    ...
    for (size_t height{0}; height < total_height; ++height) {
        ...
        while (!CheckProofOfWork(block.GetYespowerPoWHash(), block.nBits, params.GetConsensus()) ||
               !CheckProofOfWork(block.GetArgon2idPoWHash(), block.nBits, params.GetConsensus())) {
            ++block.nNonce;
            assert(block.nNonce);
        }
    }
