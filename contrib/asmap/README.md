...
be used in Dpowcoin Core with the `-asmap` option.
...
to binary before being used with Dpowcoin Core.
...
the `dpowcoin-cli getnodeaddresses` command.
...
```
dpowcoin-cli getnodeaddresses 0 > addrs.json
```
