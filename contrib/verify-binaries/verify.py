...
signature file SHA256SUMS.asc from dpowcore.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/dpowcore-project/guix.sigs/tree/main/builder-keys
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Dpowcoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"dpowcoin_verify_binaries.{args.version}"
